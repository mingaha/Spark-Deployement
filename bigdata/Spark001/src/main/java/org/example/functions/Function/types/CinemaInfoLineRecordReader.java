...
@Slf4j
@NoArgsConstructor
public class CinemaInfoLineRecordReader extends RecordReader<CinemaInfoLongWritable, CinemaInfoText> {
    ...
    private CinemaInfoLongWritable key;
    private CinemaInfoText value;
    private boolean isCompressedInput;
    private Decompressor decompressor;
    private byte[] recordDelimiterBytes;
    

    public CinemaInfoLineRecordReader(byte[] recordDelimiter) {
    ...
    public void initialize(InputSplit genericSplit, TaskAttemptContext context) throws IOException {
        ...
        if (this.start != 0L) {
            this.start += (long)this.in.readLine(new CinemaInfoText(), 0, this.maxBytesToConsume(this.start));
        }
    ...
    public boolean nextKeyValue() throws IOException {
        if (this.key == null) {
            this.key = new CinemaInfoLongWritable();
        }

        this.key.set(this.pos);
        if (this.value == null) {
            this.value = new CinemaInfoText();
        }
    ...
    public CinemaInfoLongWritable getCurrentKey() {
        return this.key;
    }

    public CinemaInfoText getCurrentValue() {
        return this.value;
    }
